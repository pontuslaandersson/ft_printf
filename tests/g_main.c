...
#include "../includes/ft_printf.h"
 ...
 void	s(void)
 {
 	...
 	*/
 	...
 	//ft_printf("'%-0*s'\n'%15c'\n", 15, str, c); // Segfault! Should be: 'Hello World    '\n'              b', getting width from "-0" and missing the star, which should still account for the '-' given, while anything but a '0' won't work with the real printf. Solution: '-' has to be parsed separately; -* and a negative argument does *not* result in a positive width, but remains negative.
	printf("0 '%-0*s'\n'%15c'\n", 15, str, c); 
 	ft_printf("'%15.s'\n", str);
 	printf("'%15.s'\n", str);
